...
{  
  ...
  //Main Block
  G4VSolid* main_block =
	  new G4Tubs("Main Block",
		  0, 100.33*mm, 177.8*mm, 0, twopi);

  G4VSolid* shave_block =
	  new G4Box("Shave Block",
		  20*mm, 88.9*mm, 100.33*mm);
  G4ThreeVector pos_shave_block1(31.115*mm, 0, 0);
  G4ThreeVector pos_shave_block2(-31.115*mm, 0, 0);

  G4VSolid* inside_cyl =
	  new G4Tubs("Inside Cylinder",
		  0, 26.046*mm, 160.02*mm, 0, twopi);
  G4ThreeVector pos_inside_cyl1(0, -3.81*mm, -64.146*mm);
  G4ThreeVector pos_inside_cyl2(0, -3.81*mm, 64.146*mm);

  G4VSolid * inside_block =
	  new G4Box("Inside Block",
		  31.115*mm, 160.02*mm, 64.146*mm);
  G4ThreeVector pos_inside_block(0, -3.81*mm, 0);

  //Side Rectangle
  G4VSolid* side_rect =
	  new G4Box("Side Rectangle",
		  3.175*mm, 31.75*mm, 5.07*mm);
  G4ThreeVector pos_side_rect(0, 0, 100.33*mm);

  //Top Stuff??
  G4VSolid* top_cyl =
	  new G4Tubs("Top Cylinder",
		  0, 6.0325*mm, 16*mm, 0, twopi);
  G4ThreeVector pos_top_cyl1(9.017*mm, 57.785*mm, -70.358*mm);
  G4ThreeVector pos_top_cyl2(0, 57.785*mm, -36.5*mm);
  G4ThreeVector pos_top_cyl3(0, 57.785*mm, 36.5*mm);
  G4ThreeVector pos_top_cyl4(-9.017*mm, 57.785*mm, 70.369*mm);

  //Center Hole
  G4VSolid* cent_hole =
	  new G4Tubs("Center Hole",
		  0, 10.1205*mm, 31.2*mm, 0, twopi);
